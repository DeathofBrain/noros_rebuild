...
//            yaml_file.close();
    ...
    void *HikCamera::HKWorkThread(void *arg) {
        ...
        unsigned char *m_pBufForSaveImage = (unsigned char *) malloc(camera::g_nPayloadSize * 3);
        ...
        while (true) {
            ...
            stConvertParam.nDstBufferSize = camera::g_nPayloadSize * 3;     //ch:输出缓存大小 | en:output buffer size
